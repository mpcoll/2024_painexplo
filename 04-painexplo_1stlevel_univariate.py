...
for model, img, event, conf in zip(models, imgs, events, confounds):
    ...
    for ev, cf in zip(event, conf):
        ...
        # Trials per pain level for test and run for expo
    ...
    for contrast in ["pain", "rating", "cues"]:
        ...
        out.to_csv(opj(outpath, "sub-" + s + "_" + contrast + "_vs_cross_nsynth.csv"))
        report.add_html(
            out.head(50).to_html(),
...
for condition in [
    ...
    # Decode with neurosynth
    out = nsynth_decoder.transform(opj(outcond, condition + "_tvals.nii")).sort_values(
